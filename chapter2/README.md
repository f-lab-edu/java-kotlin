...
## 가독성을 목표로 설계하라
🤔**가독성**이란? 
코드를 읽고 얼마나 빠르게 이해할 수 있는가를 의미한다.
가독성의 정도는 각 개발자의 **경험, 지식, 패턴 숙련도**에 따라 달라진다. 
정도는 다를지라도, **각기 다른 경험치를 가진 개발자들 모두가 이해**할 수 있는 코드가 제일 가독성이 좋은 코드이다. 

대부분의 개발자는 코틀린이 첫 언어가 아니다. 따라서 코틀린의 특징이 강한 구조나 기능을 사용하면 가독성이 떨어질 수 있다.

📝일반적이지 않은 기능이나 구조를 사용할 때 생길 수 있는 문제점은 다음과 같다
- 코드 흐름을 인지하는데 시간이 더 걸린다.
- 수정에 더 많은 시간과 생각이 걸린다.
- 디버깅 분석이 더뎌질 수 있다.
- 기능을 완벽하게 이해하지 못해 잘못된 동작을 하는 코드를 만들 수 있다.

다만 무조건 피하지 말자. 
가독성이 떨어지더라도 기능적으로 필요한 부분이라면 사용을 고려해볼만 하다.
ex. let을 사용해 연산을 아규먼트 뒤로 보내거나 데코레이터 패턴을 사용할 때

## 연산자 오버로드를 할 때는 의미에 맞게 사용하라
🤔**연산자 오버로딩**란?
기존에 있는 연산자 기능에 의미를 다시 부여.
- C언어에 있지만 자바에는 기능이 없음
- String 클래스에서 + 연산자만 내부적으로 오버로딩이 되어있는 상태

연산자 오버로드는 조심히 사용해야 한다.
기능의 의미가 헷갈리면서 오해의 소지를 줄 수 있기 때문이다.
ex.
```java
...
📝따라서 연산자 오버로드 시 다음을 고려해볼 수 있다.
- 연산자 이름과 기능의 합이 적합한가?
- 기존에 만들어진 기능과 의미나 작동 방식이 충돌하진 않는가?
- 애초에 이미 만들어진 기능이 있진 않은가?

📝오버로딩 대체제도 제시한다
- 새 일반 함수로 정의
- 톱 레벨 함수(top level function) : static 메서드

[https://velog.io/@cchloe2311/Kotlin-Kotlin엔-static-키워드가-없다](https://velog.io/@cchloe2311/Kotlin-Kotlin%EC%97%94-static-%ED%82%A4%EC%9B%8C%EB%93%9C%EA%B0%80-%EC%97%86%EB%8B%A4)

- infix 확장 함수 : 두 개의 변수 가운데 오는 함수

## Unit?을 리턴하지 말라
🤔**Unit**이란?
자바에서 void처럼 사용되는 객체
[https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/-unit/](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/-unit/)

아무 값이 없는 Nothing과 의미가 충돌되기에 void 대신 Unit으로 사용한다고 한다.
Unit? 은 객체가 null인지 아닌지 Boolean 값을 반환하도록 사용이 가능합니다.
...
이해하기 어렵고, 오해의 소지가 있으니, 사용 지양
- Boolean은 true/false을 의미
- Unit은 Unit/void를 의미

##변수 타입이 명확하지 않은 경우 확실하게 지정하라
코틀린은 타입을 추론해 지정해주는 기능이 있다.
당연히 편하지만, 처음 보는 사람은 타입을 파악하는데 시간이 걸린다.
그리고 타입이 명시되지 않아 다른 개발자가 오용할 수 있으니 주의해야 한다.

## 리시버를 명시적으로 참조하라
확장 리시버를 표현할 때 this를 사용한다.
여러 개의 리시버가 있다면 명시하는 게 좋다. 만약 외부에 있는 리시버를 사용한다면 레이블을 사용해야 한다.
...
🤔**파생 프로퍼티**란?
자바의 필드와 달리 코틀린의 프로퍼티는 데이터를 저장함과 동시에 사용자 정의 getter와 setter가 자동으로 제공된다. (물론 val은 지원하지 않는다)

캡슐화되기 때문에 다른 프로퍼티에 랩, 언랩으로 전달하는 방법이 있다.
접근자로 사용될 수 있지만 **getter, setter 용도** 외 다른 로직이 필요하다면 별도의 함수로 정의해서 사용하는 게 좋다.
왜냐면 프로퍼티는 **상태를 표현하거나 설정**할 때만 사용하는게 원칙이다. 

## 이름 있는 아규먼트를 사용하라
이해하기 어려운 아규먼트를 이름을 가진 변수로 정의해 **이해도를 높인다**
또한 명시적인 아규먼트 사용으로 **의도에 적합한 방식으로 사용**할 수 있어 신뢰도를 높인다.
...
비슷한 형태의 아규먼트가 많거나 낯선 아규먼트라면  명시하는 게 낫다.
함수 타입 파라미터의 경우에도 작동 방식을 코드로 파악하기 보다 **명시한 함수명을 보고 의도를 파악**하는게 덜 헷갈린다.

## 코딩 컨벤션을 지켜라
코틀린 코딩 컨벤션 : [https://kotlinlang.org/docs/coding-conventions.html](https://kotlinlang.org/docs/coding-conventions.html)

📝코딩 컨벤션이 완벽한 정답은 아니지만, 규격을 지킴으로써 오는 장점이 있다.
- 외부 개발자도 쉽게 이해 가능
- 코드 작동 방식을 쉽게 추측
- 프로젝트 간 코드 병합이나 이동에 용이
